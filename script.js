...
let posts =[];//Iniciamos los posts como un array vacio
...
function renderPostList() {
    ...
    posts.forEach(post => {
        ...
        <div id="editForm-${post.id}" class="editForm" style="display:none">
            <label for="editTitle">Titulo: </lable>
            <input type="text" id="editTitle-${post.id}" value="${post.title}" required>
            <label for="editBody">Comentario: </lable>
...
function editPost(id) {
    ...
    editForm.style.display = (editForm.style.display == "none") ? `block` : `none` //Esto realiza un cambio entre mostrar y ocultar el edit
}
...
function updatePost(id) {
    ...
    fetch(`${urlBase}/${id}`, {
      ...
      .catch (error => console.error("Error al  postactualizar posteo:",error))
